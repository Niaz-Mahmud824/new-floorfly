 ...
 function putPermanentEnv($key, $value)
{
 ...
 if (! function_exists('CategoryControl')) {
...
//adin view cat name
if (! function_exists('getServiceParentcategoryName')) {
	function getServiceParentcategoryName($id) 
	{ 
		return ServiceCategory::find($id);
	}
}
//adin view sub cat name
if (! function_exists('getServiceSubcategoryName')) {
	function getServiceSubcategoryName($id) 
	{ 
...
//position expire | imran


if (! function_exists('check_position_exp')) {

    function check_position_exp()
    {
        $today_date=strtotime(date('m/d/Y'));

        $proeprty_list = Properties::where('position_status',1)->where('position_exp_date','<=',$today_date)->get(); 
        ...
        { 
            ...
            \Mail::send('emails.expiry_notice', $data_email, function($message) use ($user_name,$user_email){
                $message->to($user_email, $user_name)
                ->from(getcong('site_email'), getcong('site_name'))
                ->subject(getcong('site_name').' Property Position expired notice');
            });
...
if (! function_exists('PropertyLocationName')) {
	function getPropertyLocationName($id) 
	{ 
